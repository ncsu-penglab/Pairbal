...
checkCounts <- function(cnts, p, quietly = F) {
...
computeLRcovar <- function(cnts, p = NA, quietly = F) {
	# Check count matrix
	...
	lrvars <- lapply(lcnts.lst, function(x) apply(t(x - t(lcnts)), 1, function(y) var(y)))
...
# smin/smax: quantile thresholds for the size of the randomly selected starting reference frame
InvariantSetMC <- function(cnts, min.pct = 0.9, min.cnt = 5, smin = 0.1, smax = 0.9, mc.it = NA, p = NA, lrvars = NA, seed = 100, quietly = F) {
	#################
	# 0) Check input
	...
	if (all(is.na(lrvars))) {
		...
		lrvars <- computeLRcovar(cnts, quietly = T)
	}
	...
	minima <- seed:(mc.it + seed - 1) %>% as.list %>% lapply(function(seed.it) runMC(lrvars, cands, smin.it, smax.it, seed.it))
	minima
}
...
#	quietly: Whether or not status messages should be reported to user. Default: FALSE
computeInvariantBal <- function(cnts, invariantASVs = NA, topASVs = NA, p = 1, clr = F, quietly = F) {
	### 0) Check input
	...
	ilrs <- topASVs %>% as.list %>% lapply(function(x) computeILR(cnts[invariantASVs,], cnts[x,]))
...
computePWB <- function(cnts, p = NA) {
	...
	lrs <- lapply(as.list(names(lcnts.lst)), function(x) {
